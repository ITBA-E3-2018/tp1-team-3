...
{
	int i = 1, status=PARSE_OK, parsedArgs = 0;				// i : indice para el arreglo de argumentos.
	 													// status: flag de error.
	 													// parsedArgs: contador de argumentos procesados.
	 																	    

	while ( i < argc && status != PARSE_ERROR )	// El ciclo se ejecuta hasta procesar todos los argumentos o que haya un error.
     {   
